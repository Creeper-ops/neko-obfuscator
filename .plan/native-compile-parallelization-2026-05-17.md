...
### [ ] P34: Select next native optimization from accepted post-P32 baseline

- Scope: choose the next generic architecture-level native optimization from accepted post-P32 code, not from rejected P33 artifacts. The next row must be backed by fresh accepted-source evidence and must avoid the rejected P27 nested-store, P29 static-setter, and P33 string-ref shapes unless new root-cause evidence explains their regressions.
- Required evidence: gather fresh accepted-source generated-C/runtime data after P33 rejection if artifacts were overwritten, then identify an exact invariant/path/site and validation target before implementation.
- Validation command or runtime target: to be filled before implementation.
- Completion criteria: to be filled before implementation.
