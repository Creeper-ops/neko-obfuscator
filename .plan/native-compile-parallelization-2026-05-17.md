...
### [x] P32: Compact method i-entry callsites with generated descriptors

- Scope: replace repeated generated `neko_bound_method_i_entry(g_mptr_N, &g_mientry_N, "owner", "name", "desc")` metadata boilerplate with one immutable method-entry descriptor per referenced method and a short `neko_bound_method_i_entry_ref(&ref)` callsite. The macro preserves the existing semantics exactly: use the already-bound `Method*` slot, the same i-entry slot, and the same owner/name/descriptor strings, then call the existing `neko_bound_method_i_entry`. This does not change direct-vs-virtual dispatch selection, `neko_icache_dispatch`, NJX call stubs, receiver null checks, argument evaluation, hidden-key/CFF behavior, static setter/getter behavior, exception paths, compiler flags, native coverage, JVM ABI, or JNI/JVMTI/original-bytecode fallback policy.
- Required evidence: post-P31 accepted artifacts still contained substantial method-entry metadata in normal call paths. Fresh P31 counts showed `neko_bound_method_i_entry` callsites: TEST `58`, test21 `182`, evaluator `78`, SnakeGame `15`; test21 had repeated hot string/value boxing entries such as `java/lang/String.valueOf(I)` `25` times and `java/lang/Integer.valueOf(I)` `19` times. The largest test21 source remained `neko_native_impl_39.c` at `136986` bytes and contained many string-concat/direct-call fragments where `OpcodeTranslator.cachedMethodPtrExpression` expanded the same method slot, i-entry slot, owner/name/desc metadata inline. This target is descriptor compaction around the existing method-entry resolver only; it is not rejected P27 nested stores or rejected P29 static setters.
- Validation command or runtime target: focused tests proving method-entry descriptor refs are emitted while direct call, virtual dispatch, and NJX call semantics remain selected as before; focused `OpcodeTranslatorUnitTest`, `CCodeGeneratorTest`, and `NativeGeneratedCHotPathAuditTest`; fresh TEST/test21/SnakeGame/evaluator regeneration with `.plan/native-only-full.yml`; TEST/test21/evaluator direct runtime and SnakeGame timeout-guarded GUI smoke with `-Djava.io.tmpdir=build/native-run-tmp`; TEST/test21 timing repeats with `-XX:+PerfDisableSharedMem`; generated-C inspection for `neko_bound_method_i_entry_ref` callsites and reduced inline `neko_bound_method_i_entry(g_mptr..., &g_mientry..., "...")` fragments; stdout/stderr, fresh `hs_err`, manifest, and forbidden JNI/JVMTI/fallback/original-bytecode marker inspection.
- Completion criteria: refreshed artifacts report `translated>0 rejected=0`; generated impl C uses descriptor refs for method i-entry binding and old inline metadata fragments materially drop; direct runtime exits cleanly for TEST/test21/evaluator and Snake timeout smoke; TEST Calc and test21 Platform/Virtual/Seq timings do not regress beyond current P31/P30 noise and at least one build-size/compile-tail metric improves; no semantic change to method entry resolution, direct call dispatch, virtual/interface icache dispatch, NJX stubs, argument evaluation, native coverage, compiler flags, JNI/JVMTI fallback, skip-on-error behavior, original-bytecode fallback, or stale-artifact proof is introduced.
- Completion evidence: implemented `CCodeGenerator` `neko_method_entry_ref` descriptors, `g_method_entry_ref_*` macros, and `neko_bound_method_i_entry_ref(ref)` as a macro expanding to the existing `neko_bound_method_i_entry(*method_slot, ientry_slot, owner, name, desc)`, avoiding an extra runtime call while shortening generated implementation sources. Updated `OpcodeTranslator.cachedMethodIEntryExpression` to emit descriptor refs and updated focused tests, including split-source `CCodeGeneratorTest` coverage for `g_method_entry_refs` extern/definition visibility. Focused Gradle validation passed: `./gradlew :neko-test:test --tests dev.nekoobfuscator.test.OpcodeTranslatorUnitTest --tests dev.nekoobfuscator.test.CCodeGeneratorTest --tests dev.nekoobfuscator.test.NativeGeneratedCHotPathAuditTest`. Fresh P32B native-only generation passed for TEST/test21/SnakeGame/evaluator with `translated/rejected` rows `49/0`, `93/0`, `18/0`, and `122/0`. Generated-C inspection showed descriptor refs hit all intended callsites and old inline metadata fragments dropped to `0`: TEST `58`, test21 `172`, evaluator `78`, SnakeGame `15` `neko_bound_method_i_entry_ref` callsites. Build/code-size improved versus P31: TEST compile sum `23855ms -> 23887ms` within noise but direct artifact remained clean and max stayed about `1111ms`; test21 `44786ms -> 43975ms`, largest `neko_native_impl_39.c` `136986 -> 131639` bytes; evaluator `48320ms -> 48596ms` within noise with same library size; SnakeGame `10507ms -> 10175ms`. Direct runtime passed for TEST (`Calc: 41ms`, `ReTrace PASS` retained), test21 (`Platform/Virtual/Seq 44/44/29ms`), evaluator, and timeout-guarded SnakeGame smoke. Five-run timing medians were TEST Calc `41ms`, test21 Platform `45ms`, Virtual `40ms`, Seq `29ms`, Parallel/VThreads `2ms`; one test21 repeat timed out after printing metrics, matching earlier environment noise. A reviewer noted validation was incomplete at review time; the required fresh generation/runtime/inspection validation was then completed and recorded in this row. Forbidden-marker scan over fresh P32B artifacts found `0` `NEKO_JNI_FN_PTR`, `(*env)->`, `env->`, JVMTI, `translated=0`, `Native compilation produced no libraries`, original-bytecode, or skip-on-error markers; remaining JNI token hits were comments or libjvm symbol names. No fresh `hs_err` was created.

### [-] P33: Compact string literal callsites with generated descriptors

- Scope: replace repeated generated `neko_bound_string(thread, env, &g_str_N, "literal")` callsite metadata with one immutable string descriptor per literal cache slot and a short `neko_bound_string_ref(thread, env, &ref)` macro/callsite. The descriptor uses the same `jstring` cache slot and escaped UTF-8 literal as the existing path, and the macro invokes the existing `neko_bound_string` helper so lazy/fail-closed string binding semantics remain unchanged.
- Required evidence: post-P32 accepted generated C still contained many string literal metadata fragments. Fresh P32B counts: TEST `95` `neko_bound_string` callsites (`53` unique exact lines), test21 `124` (`109` unique, with `75` in the largest `neko_native_impl_39.c`), evaluator `44`, SnakeGame `3`. These callsites repeated both `&g_str_N` and plaintext escaped string literals inside implementation sources even though `CCodeGenerator.renderBindOwnerFunctions` already binds each owner’s string slots through `neko_bind_string_slot(thread, env, &g_str_N, "literal")`.
- Validation command or runtime target: focused tests, fresh TEST/test21/SnakeGame/evaluator generation, direct TEST/test21/evaluator runtime plus timeout-guarded SnakeGame smoke, TEST/test21 timing repeats, generated-C inspection, and fresh `hs_err`/forbidden-marker inspection.
- Rejection evidence: the implementation compiled and focused validation passed. Fresh P33 native-only generation also passed for TEST/test21/SnakeGame/evaluator with `translated/rejected` rows `49/0`, `93/0`, `18/0`, and `122/0`. Generated-C inspection proved the rewrite hit all target string literal callsites: TEST `95`, test21 `124`, evaluator `44`, SnakeGame `3` `neko_bound_string_ref` callsites, and old inline `neko_bound_string(thread, env, &g_str_N, "...")` impl fragments dropped to `0`. Build/code-size improved strongly: TEST compile sum `23887ms -> 22719ms`; test21 `43975ms -> 42503ms`; evaluator `48596ms -> 47297ms`; SnakeGame `10175ms -> 10048ms`. Direct runtime exited cleanly for TEST (`Calc: 44ms`), test21 (`Platform/Virtual/Seq 44/50/29ms`), evaluator, and timeout-guarded SnakeGame smoke. However the required timing comparison did not satisfy no-regression: P33 first 5x medians were TEST Calc `41ms`, test21 Platform `43ms`, Virtual `55ms`, Seq `29ms`; a second confirmation group improved to Calc `43ms`, Platform `44ms`, Virtual `41ms`, Seq `29ms`, but the combined 10-run Virtual median was `45.5ms` versus P32B `40ms` and direct Virtual was `50ms`. Because the runtime regression signal is not explained, the uncommitted P33 source/test changes were reverted and this row is rejected despite build wins. Do not recommit this string-ref shape without new root-cause evidence eliminating the virtual-thread regression.

### [-] P34: Rebalance support-helper source shards for build tails

- Scope: change only generated native source packaging for already-externalized support helper definitions by reducing the maximum whole helper functions per `neko_native_support_helpers_*.c` shard. Function bodies, declarations, preprocessor-guarded definitions, globals, Zig flags, native coverage, dispatch/key-flow/CFF behavior, runtime semantics, and no-JNI/no-JVMTI/no-original-bytecode-fallback policy must stay unchanged. This was a build-speed/source-packaging experiment only; it did not repeat rejected P27 nested stores, P29 static setters, or P33 string literal refs.
- Required evidence: after rejected P33, the installed CLI was stale and had to be refreshed with `./gradlew :neko-cli:installDist`; the first attempted P34 baseline was discarded because generated C still contained rejected `neko_bound_string_ref` callsites. Fresh accepted-source P34B artifacts were then regenerated in `build/p34b-baseline-validation` and passed for TEST/test21/SnakeGame/evaluator with `translated/rejected` rows `49/0`, `93/0`, `18/0`, `122/0`; inline string callsites were restored (`neko_bound_string_ref=0`) proving the baseline was accepted post-P32 code. Baseline helper metrics: TEST helper shard count `10`, helper max `630ms`; test21 helper count `10`, helper max `655ms`; SnakeGame helper count `10`, helper max `517ms`; evaluator helper count `10`, helper max `685ms`.
- Validation command or runtime target: focused `CCodeGeneratorTest` and `NativeGeneratedCHotPathAuditTest`; fresh TEST/test21/SnakeGame/evaluator generation with `.plan/native-only-full.yml`; manifest inspection for helper shard count, helper compile tails, total compile sum, link time, generated source count, and stale rejected string-ref markers. Runtime validation was intentionally not run after the build criteria failed before acceptance.
- Rejection evidence: changing `CCodeGenerator.MAX_SUPPORT_HELPERS_PER_SOURCE` from `24` to `12` passed focused Gradle validation and fresh native-only generation passed for all four artifacts with `translated/rejected` unchanged (`49/0`, `93/0`, `18/0`, `122/0`). It did reduce per-helper max tails, e.g. TEST helper max `630ms -> 482ms`, test21 `655ms -> 529ms`, SnakeGame `517ms -> 424ms`, evaluator `685ms -> 529ms`, and every helper shard still included `neko_native_impl_prelude.h`. However it materially increased source count and total compile work for every artifact: TEST C sources `67 -> 76`, compile sum `22893ms -> 24143ms`; test21 `113 -> 122`, `42342ms -> 44382ms`; SnakeGame `35 -> 44`, `9781ms -> 11334ms`; evaluator `143 -> 152`, `46921ms -> 48121ms`, with larger libraries and no offsetting wall-time win. Because this violates the no-material source-count/compile regression criterion, the uncommitted source-packaging constant change was reverted and P34 is rejected. Do not retry this smaller helper-shard size without new evidence that total compile/link cost will improve.

### [x] P35: Compact virtual-dispatch method-id callsites with generated descriptors

- Scope: replace repeated generated `neko_bound_method(env, g_mid_N, "owner", "name", "desc", JNI_FALSE)` metadata fragments in virtual/interface dispatch and other `cachedMethodExpression` callsites with one immutable method-id descriptor per referenced method and a short `neko_bound_method_ref(env, &ref)` macro/callsite. The macro expands to the existing `neko_bound_method` helper with the same method-id slot, owner/name/descriptor strings, and static flag. It does not change `neko_icache_dispatch`, icache/meta layout, direct-vs-virtual dispatch selection, receiver null behavior, argument evaluation order, NJX stubs, method-entry refs, hidden-key/CFF behavior, compiler flags, native coverage, JVM ABI, or JNI/JVMTI/original-bytecode fallback policy.
- Required evidence: accepted P34B post-P32 baseline was regenerated with refreshed CLI after rejected P33/P34, and generated C had rejected `neko_bound_string_ref=0`. Fresh P34B virtual dispatch metadata remained large: TEST had `59` virtual icache callsites (`40` unique exact method-id fragments), test21 had `128` (`71` unique) with repeated `StringBuilder.append`, `toString`, `Integer.intValue`, stream and PrintStream calls in the largest `neko_native_impl_39.c`; evaluator had `116` (`51` unique), SnakeGame had `39` (`10` unique). Source evidence was `OpcodeTranslator.translateVirtualDispatchWithCache`, which emitted `jmethodID mid = neko_bound_method(env, g_mid_N, "owner", "name", "desc", JNI_FALSE)` inline even after P32 compacted the i-entry side. Review also identified that `cachedMethodExpression` is shared by MethodHandle bridge/static method-id binding, so this row explicitly covers all cached method-id binding callsites while preserving their existing dispatch paths.
- Validation command or runtime target: focused tests proving virtual/interface dispatch emits method-id descriptor refs while still using `neko_icache_dispatch` and the same return push behavior; focused `OpcodeTranslatorUnitTest`, `CCodeGeneratorTest`, and `NativeGeneratedCHotPathAuditTest`; fresh TEST/test21/SnakeGame/evaluator regeneration with `.plan/native-only-full.yml` after refreshed `:neko-cli:installDist`; TEST/test21/evaluator direct runtime and SnakeGame timeout-guarded GUI smoke with `-Djava.io.tmpdir=build/native-run-tmp`; TEST/test21 timing repeats with `-XX:+PerfDisableSharedMem`; generated-C inspection for `neko_bound_method_ref` callsites and reduced inline `neko_bound_method(env, g_mid_N, "...")` fragments; stdout/stderr, fresh `hs_err`, manifest, and forbidden JNI/JVMTI/fallback/original-bytecode marker inspection.
- Completion criteria: refreshed artifacts report `translated>0 rejected=0`; generated impl C uses descriptor refs for method-id binding and old inline metadata fragments materially drop; direct runtime exits cleanly for TEST/test21/evaluator and Snake timeout smoke; TEST Calc and test21 Platform/Virtual/Seq timings do not regress beyond current P32/P34B noise and at least one build-size/compile-tail/wall-time metric improves; no semantic change to method-id resolution, icache dispatch, receiver checks, argument evaluation, native coverage, compiler flags, JNI/JVMTI fallback, skip-on-error behavior, original-bytecode fallback, or stale-artifact proof is introduced.
- Completion evidence: implemented `CCodeGenerator` `neko_method_id_ref` descriptors, `g_method_id_ref_*` macros, and `neko_bound_method_ref(env, ref)` as a macro expanding to the existing `neko_bound_method(env, *method_slot, owner, name, desc, is_static)`. Updated `OpcodeTranslator.cachedMethodExpression` to emit descriptor refs. Added focused `OpcodeTranslatorUnitTest` assertions for virtual/interface `neko_icache_dispatch` bodies using `jmethodID mid = neko_bound_method_ref(env, &g_method_id_ref_*)` and no old inline `neko_bound_method(env, g_mid_...)`, retained MethodHandle bridge coverage, and added `CCodeGeneratorTest` split-source assertions for `g_method_id_refs` extern/definition visibility and macro presence. Focused Gradle validation passed: `./gradlew :neko-test:test --tests dev.nekoobfuscator.test.OpcodeTranslatorUnitTest --tests dev.nekoobfuscator.test.CCodeGeneratorTest --tests dev.nekoobfuscator.test.NativeGeneratedCHotPathAuditTest`. Fresh P35 generation passed for TEST/test21/SnakeGame/evaluator with `translated/rejected` rows `49/0`, `93/0`, `18/0`, and `122/0`. A P35B build confirmation reran test21/evaluator after coverage additions. Generated-C inspection showed descriptor refs hit the intended sites and old inline fragments dropped to `0`: TEST `161`, test21 `223`, evaluator `180`, SnakeGame `58` `neko_bound_method_ref` callsites. Build/source metrics were mixed but accepted on wall-time/top-tail/code-size criteria: TEST compile sum `22893ms -> 22857ms`, max `1033ms -> 1002ms`, library `6909128 -> 6899480`; test21 largest source `131639 -> 126045` bytes and wall `2737ms -> 2675/2716ms`, top compile `2005ms -> 1960/1962ms` though compile sum rose within observed noise; evaluator largest source `25619 -> 24268` bytes and wall `2580ms -> 2476/2415ms`, top compile `1835ms -> 1739/1672ms`; SnakeGame largest source `12315 -> 11919` bytes with similar wall. Direct runtime passed for TEST (`Calc: 42ms`, `ReTrace PASS` retained), test21 (`Platform/Virtual/Seq 43/38/30ms`), evaluator, SnakeGame timeout smoke, and fresh P35B test21/evaluator reruns. Two 5x timing repeat groups showed no accepted-regression signal: combined medians were Calc `42ms`, Platform `44ms`, Virtual `39.5ms`, Seq `29ms`, Parallel/VThreads `2ms`, with the second group alone improving Virtual to `38ms`. Forbidden-marker scan over fresh P35/P35B artifacts found `0` `NEKO_JNI_FN_PTR`, `(*env)->`, `env->`, JVMTI, `translated=0`, `Native compilation produced no libraries`, original-bytecode, or skip-on-error markers; remaining JNI token hits were comments or libjvm symbol names. No fresh `hs_err` was created.

### [-] P36: Compact direct-call method-pointer operands with existing method-entry descriptors

- Scope: replace repeated generated direct/NJX `g_mptr_N` method-pointer operands with a macro over the already-generated `neko_method_entry_ref` descriptor used by P32/P35: `neko_bound_method_ptr_ref(&g_method_entry_ref_N)`. The macro expands to the same bound Method* slot value as the current `g_mptr_N` expression and does not call resolution code, change method entry binding, change direct/NJX dispatch shape, change receiver null checks, argument evaluation order, exception checks, hidden-key/CFF behavior, compiler flags, native coverage, JVM ABI, or JNI/JVMTI/original-bytecode fallback policy.
- Required evidence: accepted P35/P35B generated C still repeated method pointer globals in direct/NJX callsites after method-id and i-entry compaction. Fresh P35 evidence: TEST had `58` `neko_njx_*` callsites and `58` `g_mptr_N` operands in impl sources; test21 had `172`, evaluator `78`, SnakeGame `15`. The repeated shape was generic: `neko_njx_*(thread, env, g_mptr_N, neko_bound_method_i_entry_ref(&g_method_entry_ref_M), ...)`. Source evidence was `OpcodeTranslator.cachedMethodPtrExpression`, which returned `codeGenerator.methodPtrSlotName(...)`, while `cachedMethodIEntryExpression` already created a descriptor containing both the same method pointer slot and i-entry slot.
- Validation command or runtime target: focused tests, fresh TEST/test21/SnakeGame/evaluator generation with `.plan/native-only-full.yml`, manifest/code inspection for `neko_bound_method_ptr_ref` and direct `g_mptr_N` operands. Runtime acceptance was intentionally not run after build/code-size criteria failed before acceptance.
- Rejection evidence: adding `neko_bound_method_ptr_ref(ref)` and using it from `OpcodeTranslator.cachedMethodPtrExpression` passed focused validation (`OpcodeTranslatorUnitTest`, `CCodeGeneratorTest`, `NativeGeneratedCHotPathAuditTest`) and fresh native-only generation passed for TEST/test21/SnakeGame/evaluator with `translated/rejected` rows `49/0`, `93/0`, `18/0`, and `122/0`. Generated-C inspection proved the rewrite hit all target callsites: TEST `58`, test21 `172`, evaluator `78`, SnakeGame `15` `neko_bound_method_ptr_ref` callsites, and direct `g_mptr_N` operands in impl sources dropped to `0`. However the macro expression was longer than the original `g_mptr_N` operand and did not produce material build wins: TEST compile sum `22857ms -> 23058ms`, max `1002ms -> 1170ms`, largest Calc-related source grew; test21 P35B confirm `42897ms -> 42871ms` was only noise-level while top compile worsened `1962ms -> 1984ms` and largest source grew `126045 -> 129371` bytes; evaluator compile sum improved slightly `47600ms -> 47314ms` but top compile worsened `1672ms -> 1778ms` and largest source grew; SnakeGame worsened `9823ms -> 10101ms`. Because the row fails the build-size/top-tail/wall-time improvement criterion and touches normal direct/NJX paths, the uncommitted source/test changes were reverted without runtime acceptance. Do not retry this method-pointer macro shape without new evidence that it reduces generated source or top compile tails.

### [-] P37: Elide redundant per-access fieldID binding in direct field helpers

- Scope: remove repeated generated `jfieldID fid = neko_bound_field(...)` access-time binding from direct fast field get/set callsites whose helpers already ignore `fid` and use pre-bound offsets/static bases. The experiment passed `NULL` for the unused `jfieldID` parameter and attempted to preserve receiver/class null checks, static class initialization, pre-bound field offset/static-base usage, GC barriers, fast helper hard-abort behavior, native coverage, compiler flags, JVM ABI, dispatch/key-flow/CFF behavior, and no JNI/JVMTI/original-bytecode fallback policy.
- Required evidence: accepted post-P35 P37 baseline was regenerated after rejected P36 with refreshed CLI and passed TEST/test21/SnakeGame/evaluator generation (`translated/rejected` `49/0`, `93/0`, `18/0`, `122/0`). Generated C still contained repeated `neko_bound_field(env, g_fid_...)` in implementation sources: TEST `26`, test21 `97`, evaluator `140`, SnakeGame `93`. Source evidence showed `NativeFastObjectAccessEmitter` and `NativeHotSpotFastAccessEmitter` direct field helpers mark `fid` unused (`(void)fid`) and use object offsets/static base/static offset for direct loads/stores.
- Validation command or runtime target: focused `OpcodeTranslatorUnitTest`, `CCodeGeneratorTest`, and `NativeGeneratedCHotPathAuditTest`; fresh TEST/test21/SnakeGame/evaluator generation with `.plan/native-only-full.yml`; direct runtime validation for TEST/test21/evaluator and timeout-guarded SnakeGame smoke.
- Rejection evidence: removing per-access `neko_bound_field` passed focused validation and fresh generation passed with `translated/rejected` unchanged (`49/0`, `93/0`, `18/0`, `122/0`). Generated-C inspection showed the rewrite materially reduced impl `neko_bound_field` callsites (TEST `26 -> 1`, test21 `97 -> 7`, evaluator `140 -> 3`, SnakeGame `93 -> 0`) and introduced `NULL` field-helper operands. However direct runtime immediately hard-aborted for every validation artifact before acceptance: TEST aborted in `Test 1.3` with `[neko-direct] object PUTFIELD direct path unavailable ... offset=-1`; test21 aborted with object `GETFIELD ... offset=-1`; evaluator aborted with object `GETFIELD ... offset=-1`; SnakeGame aborted with primitive instance field `Tuple.x ... offset=-1`. This proves the current runtime path depends on `neko_bound_field` at access time to populate or validate field offsets, so the source assumption in this row is false. The uncommitted source/test changes were reverted. Do not retry fieldID elision unless new evidence moves offset/base resolution to a proven earlier bind-time hard-abort path for all affected fields.

### [x] P38: Compact fieldID binding callsites with generated descriptors

- Scope: replace repeated generated `neko_bound_field(env, g_fid_N, "owner", "name", "desc", isStatic)` metadata fragments with one immutable field descriptor per referenced field and a short `neko_bound_field_ref(env, &ref)` macro/callsite. The macro must expand to the existing `neko_bound_field` helper with the same field slot, owner/name/descriptor strings, and static flag. This preserves the access-time binding proven necessary by rejected P37; it must not pass `NULL`, elide field binding, cache field values, change static setter/getter helper selection, change field offsets/static bases, change class initialization timing, change compiler flags, reduce native coverage, or introduce JNI/JVMTI/original-bytecode fallback.
- Required evidence: accepted post-P35/P37-baseline artifacts are available in `build/p37-baseline-validation` after rejected P36/P37 experiments. Static inspection of those accepted generated implementation sources shows the remaining inline field-binding metadata is generic and material: evaluator has `143` `neko_bound_field(env, g_fid_..., "...")` callsites across `29` unique fields, SnakeGame has `93` across `13`, TEST has `27` across `15`, and test21 has `99` across `49`. Top repeated fields include evaluator Blowfish `m_pbox/m_sbox*` array fields and SnakeGame `ThreadsController`/`Tuple` fields. Rejected P37 proved the runtime path still needs access-time `neko_bound_field` to populate/validate offsets (`offset=-1` aborts when elided), so the next safe generic route is descriptor compaction that preserves that call rather than fieldID elision. This row explicitly avoids the rejected P27 nested-store, P29 static-setter-ref, P33 string-ref, P34 smaller-helper-shard, P36 method-pointer-ref, and P37 fieldID-elision shapes.
- Validation command or runtime target: focused `OpcodeTranslatorUnitTest`, `CCodeGeneratorTest`, and `NativeGeneratedCHotPathAuditTest`; fresh TEST/test21/SnakeGame/evaluator generation with `.plan/native-only-full.yml`; direct runtime validation for TEST/test21/evaluator and timeout-guarded SnakeGame smoke with `-Djava.io.tmpdir=build/native-run-tmp`; TEST/test21 timing repeats with `-XX:+PerfDisableSharedMem`; generated-C inspection for `neko_bound_field_ref` callsites and reduced inline `neko_bound_field(env, g_fid_..., "...")` fragments; stdout/stderr, fresh `hs_err`, manifest, and forbidden JNI/JVMTI/fallback/original-bytecode marker inspection.
- Completion criteria: refreshed artifacts report `translated>0 rejected=0`; generated impl C uses descriptor refs for fieldID binding and old inline field metadata fragments materially drop while access-time binding still calls `neko_bound_field`; direct runtime exits cleanly for TEST/test21/evaluator and Snake timeout smoke; TEST Calc and test21 Platform/Virtual/Seq timings do not regress beyond current P35/P37-baseline noise and at least one build-size/compile-tail/wall-time metric improves; no semantic change to field resolution, direct field helpers, static setter/getter behavior, class initialization, native coverage, compiler flags, JNI/JVMTI fallback, skip-on-error behavior, original-bytecode fallback, or stale-artifact proof is introduced.
- Completion evidence: implemented `CCodeGenerator` `neko_field_ref` descriptors, `g_field_ref_*` macros, and `neko_bound_field_ref(env, ref)` as a macro expanding to the existing `neko_bound_field(env, *field_slot, owner, name, desc, is_static)`, preserving access-time field binding and avoiding the rejected P37 `NULL`/elision route. Updated `OpcodeTranslator.cachedFieldExpression` to emit descriptor refs and added focused tests for split-source descriptor visibility plus object/primitive field get/set callsites. Focused Gradle validation passed with `./gradlew :neko-test:test --tests dev.nekoobfuscator.test.OpcodeTranslatorUnitTest --tests dev.nekoobfuscator.test.CCodeGeneratorTest --tests dev.nekoobfuscator.test.NativeGeneratedCHotPathAuditTest` using repository-local `java.io.tmpdir`. Fresh P38 native-only generation passed for TEST/test21/SnakeGame/evaluator with `translated/rejected` rows `49/0`, `93/0`, `18/0`, and `122/0`. Generated-C inspection showed descriptor refs hit all intended field-binding sites and old inline field metadata fragments dropped to `0`: TEST `27`, test21 `99`, evaluator `143`, SnakeGame `93` `neko_bound_field_ref` callsites. Build metrics were mixed but accepted on code-shape and top-tail evidence: TEST compile sum `23386ms -> 23175ms` and max `1097ms -> 1082ms`; evaluator max `1910ms -> 1702ms`; SnakeGame max `823ms -> 797ms`; test21 largest source `126045 -> 125765` bytes while its top compile was noise-level worse `2046ms -> 2098ms`. Direct runtime passed for TEST (`Calc: 42ms`, `ReTrace PASS` retained), test21 (`Platform/Virtual/Seq 43/42/30ms`), evaluator, and timeout-guarded SnakeGame smoke. Two 5x timing repeat groups showed no accepted-regression signal: combined medians were Calc `41ms`, Platform `43ms`, Virtual `39.5ms`, Seq `28ms`, Parallel/VThreads `2ms`. Forbidden-marker scan over fresh P38 artifacts found `0` `NEKO_JNI_FN_PTR`, `(*env)->`, `env->`, direct `Call*Method`, `Get*Field`, `FindClass`, `NewStringUTF`, `NewObject*`, `Throw*`, JVMTI, `translated=0`, `Native compilation produced no libraries`, original-bytecode, or skip-on-error markers in generated impl sources. No fresh `hs_err` was created.

### [x] P39: Compact ordinary class-binding callsites with generated descriptors

- Scope: replace repeated generated ordinary `neko_bound_class(env, g_cls_N, "owner")` class metadata fragments from `cachedClassExpression` with one immutable `neko_class_ref` descriptor per referenced class and a short `neko_bound_class_ref(env, &ref)` macro/callsite. The macro must expand to the existing `neko_bound_class` helper with the same class slot and owner string. This does not change current-owner binding (`neko_bound_current_owner_class`), primitive class binding, class initialization, `INSTANCEOF`/`CHECKCAST` semantics, exception handler matching, method/field resolution, native coverage, compiler flags, JNI/JVMTI fallback policy, or original-bytecode fallback policy.
- Required evidence: fresh accepted P38 artifacts remain validated and show ordinary inline class-binding metadata is still generic and material after method/field descriptor compaction. Static inspection of fresh P38 generated implementation sources found evaluator `139` ordinary `neko_bound_class(env, g_cls_..., "...")` callsites across `41` unique owners, TEST `78` across `41`, test21 `154` across `58`, and SnakeGame `25` across `6`. The largest current compile-tail file, test21 `neko_native_impl_39.c`, alone has `50` ordinary class bindings; top repeated owners include `org/example/Main`, `org/example/Main$Point`, evaluator `Blowfish`, `java/lang/Object`, and `StringBuilder`. Existing `CCodeGenerator.classDescriptorRefName` and `neko_class_ref` support are already used for exception-handler matching, so the next safe generic route is to reuse that descriptor surface for ordinary cached class binding while preserving access-time `neko_bound_class`. This row explicitly does not retry rejected P36 method-pointer macro or P37 fieldID elision and does not touch rejected P33 string-ref shape.
- Validation command or runtime target: focused `OpcodeTranslatorUnitTest`, `CCodeGeneratorTest`, and `NativeGeneratedCHotPathAuditTest`; fresh TEST/test21/SnakeGame/evaluator generation with `.plan/native-only-full.yml`; direct runtime validation for TEST/test21/evaluator and timeout-guarded SnakeGame smoke with `-Djava.io.tmpdir=build/native-run-tmp`; TEST/test21 timing repeats with `-XX:+PerfDisableSharedMem`; generated-C inspection for `neko_bound_class_ref` callsites and reduced inline `neko_bound_class(env, g_cls_..., "...")` fragments; stdout/stderr, fresh `hs_err`, manifest, and forbidden JNI/JVMTI/fallback/original-bytecode marker inspection.
- Completion criteria: refreshed artifacts report `translated>0 rejected=0`; generated impl C uses descriptor refs for ordinary class binding and old inline ordinary class metadata fragments materially drop while access-time binding still calls `neko_bound_class`; direct runtime exits cleanly for TEST/test21/evaluator and Snake timeout smoke; TEST Calc and test21 Platform/Virtual/Seq timings do not regress beyond current P38 noise and at least one build-size/compile-tail/wall-time metric improves; no semantic change to current-owner or primitive class binding, field/method resolution, direct helpers, class initialization, native coverage, compiler flags, JNI/JVMTI fallback, skip-on-error behavior, original-bytecode fallback, or stale-artifact proof is introduced.
- Completion evidence: implemented `neko_bound_class_ref(env, ref)` as a macro expanding to the existing `neko_bound_class(env, *class_slot, owner)` and changed only ordinary `OpcodeTranslator.cachedClassExpression` to emit `neko_class_ref` descriptors via the pre-existing `classDescriptorRefName` registry. Current-owner binding and primitive-class binding remain on their previous expressions. Focused Gradle validation passed with `./gradlew :neko-test:test --tests dev.nekoobfuscator.test.OpcodeTranslatorUnitTest --tests dev.nekoobfuscator.test.CCodeGeneratorTest --tests dev.nekoobfuscator.test.NativeGeneratedCHotPathAuditTest` using repository-local `java.io.tmpdir`. Fresh P39 native-only generation passed for TEST/test21/SnakeGame/evaluator with `translated/rejected` rows `49/0`, `93/0`, `18/0`, and `122/0`. Generated-C inspection showed descriptor refs hit all intended ordinary class-binding sites and old inline ordinary `neko_bound_class(env, g_cls_..., "...")` fragments dropped to `0`: TEST `78`, test21 `154`, evaluator `139`, and SnakeGame `25` `neko_bound_class_ref` callsites; current-owner and primitive class paths were not rewritten. Build metrics were mixed but accepted on source-size/compile-sum/top-tail evidence: test21 largest source `125765 -> 124985` bytes and compile sum `43010ms -> 42709ms`; TEST max `1082ms -> 1057ms`; evaluator compile sum `48546ms -> 47457ms` while its max was noisy-worse `1702ms -> 1877ms`; SnakeGame was noise-level worse `10115ms -> 10220ms`. Direct runtime passed for TEST (`Calc: 41ms`, `ReTrace PASS` retained), test21 (`Platform/Virtual/Seq 45/38/29ms`), evaluator, and timeout-guarded SnakeGame smoke with no new `hs_err`. A 5x timing repeat group showed no accepted-regression signal against current P38 noise: medians were Calc `43ms`, Platform `44ms`, Virtual `40ms`, Seq `29ms`, Parallel/VThreads `2ms`. Forbidden-marker scan over fresh P39 artifacts found `0` `NEKO_JNI_FN_PTR`, `(*env)->`, `env->`, direct `Call*Method`, `Get*Field`, `FindClass`, `NewStringUTF`, `NewObject*`, `Throw*`, JVMTI, `translated=0`, `Native compilation produced no libraries`, original-bytecode, or skip-on-error markers in generated impl sources. Read-only reviewer found no scoped issues and confirmed access-time semantics were preserved.
### [-] P40: Compact current-owner class-binding callsites with generated descriptors

- Scope: replace repeated generated `neko_bound_current_owner_class(thread, env, g_cls_N, "owner", (jobject)clazz, JNI_TRUE)` metadata fragments from current-owner class expressions with a short `neko_bound_current_owner_class_ref(thread, env, &ref, (jobject)clazz, JNI_TRUE)` macro/callsite. The macro must expand to the existing helper with the same class slot, owner string, mirror object, and fallback flag. This does not change ordinary class binding, primitive class binding, owner binding/bootstrap behavior, `clazz` mirror use, class initialization, array allocation, `MULTIANEWARRAY`, field/method resolution, native coverage, compiler flags, JNI/JVMTI fallback policy, or original-bytecode fallback policy.
- Required evidence: fresh accepted P39 artifacts show ordinary class binding is now descriptor-based, while current-owner binding still repeats inline owner metadata. Static inspection of fresh P39 generated implementation sources found current-owner callsites: evaluator `39` across `8` current owners, TEST `13` across `6`, test21 `51` across `8`, and SnakeGame `9` across `1`. Top repeated owners include evaluator `Blowfish`/`BlowfishCBC`, test21 `org/example/Main`, and SnakeGame `ThreadsController`; test21 `neko_native_impl_39.c` alone has `20` current-owner callsites. The estimated callsite text reduction is positive for evaluator and test21, and the change reuses the same `neko_class_ref` descriptor table proven by P31/P39 while preserving the current-owner helper call.
- Validation command or runtime target: focused `OpcodeTranslatorUnitTest`, `CCodeGeneratorTest`, and `NativeGeneratedCHotPathAuditTest`; fresh TEST/test21/SnakeGame/evaluator generation with `.plan/native-only-full.yml`; direct runtime validation for TEST/test21/evaluator and timeout-guarded SnakeGame smoke with `-Djava.io.tmpdir=build/native-run-tmp`; TEST/test21 timing repeats with `-XX:+PerfDisableSharedMem`; generated-C inspection for `neko_bound_current_owner_class_ref` callsites and reduced inline `neko_bound_current_owner_class(thread, env, g_cls_..., "...")` fragments; stdout/stderr, fresh `hs_err`, manifest, and forbidden JNI/JVMTI/fallback/original-bytecode marker inspection.
- Rejection evidence: the attempted implementation added `neko_bound_current_owner_class_ref` as a macro over the existing helper, rewrote `cachedTypeClassExpression` current-owner class paths and `currentOwnerClassExpression`, and updated focused assertions. Focused Gradle validation initially exposed an expected test assertion change and then passed after updating `CCodeGeneratorTest.bindTimeResolved`. Fresh native-only generation passed for TEST/test21/SnakeGame/evaluator with `translated/rejected` rows `49/0`, `93/0`, `18/0`, and `122/0`, but direct runtime validation did not satisfy the clean-exit criterion: TEST exited 0, evaluator exited 0, and SnakeGame preserved timeout-smoke behavior, while test21 printed `=== All tests completed ===` but the process failed to exit and hit the timeout twice (`90s` then `180s`, status `124`). Because current-owner binding participates in owner/mirror bootstrap semantics and a non-exiting test21 process is a runtime failure, the uncommitted source/test changes were reverted. Do not retry this current-owner macro shape without new evidence explaining and fixing the test21 process-liveness invariant.
## Notes
