...
### [ ] P30: Select next native optimization from post-P29 accepted baseline

- Scope: choose the next generic architecture-level native optimization from accepted post-P28 code, not from rejected P29 artifacts. The next row must be backed by fresh evidence and must avoid the rejected P27 nested-store and P29 static-setter callsite shapes unless new root-cause evidence explains their regressions.
- Required evidence: gather fresh accepted-source generated-C/runtime data after P29 rejection if artifacts were overwritten, then identify an exact invariant/path/site and validation target before implementation.
